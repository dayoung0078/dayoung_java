...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        * 부모(super)가 가지고 있는 재산(자바에서는 클래스가 가지는 멤버)을 자식이 물려받는 의미이다. //멤버? 필드랑 메소드 모두 포함
        ...
        * 물려받아서 자신의 것 처럼 사용하는 것 뿐 아니라 추가적인 멤버도 작성이 가능하다. (상속받은 기능을 절대로 없애거나 줄일 순 없다)
        * 특히 메소드 재정의(overriding)라는 기술을 이용해서 부모가 가진 메소드를 재정의 하는 것도 가능하다.
        *
        * 메소드 재정의(overriding)이란 무고가 가지는 메소드 선언부를 그대로 사용하면서
        * 자식클래스가 정의한 메소드대로 동작하도록 구현 몸체 부분을 새롭게 다시 작성하는 기술이다. /틀만 바꾸는건가?
        * 메소드 재정의를 하면 메소드를 호출할 시 재정의한 메소드가 우선으로 동작하게 된다.
        *
        *
        * 이러한 상속이라는 기술을 사용하게 되면 얻게되는 이점은 크게 두 가지로 볼 수 잇다.
        * 1. 새로운 클래스를 작성할 시 기존에 작성한 클래스를 재사용할 수 있다. / ex) 자동차의 기능을 다 받고 겉 몸체만 바뀌는것.
        *   1-1. 재사용 시 생산성을 크게 향상시킬 수 있다. (새롭게 작성하는것보다 빠르다.) / 기존에 있는 메소드를 값만 조정하면 업그레이드 할 수 있음.
        *   1-2. 공통적으로 사용하는 코드가 부모클래스에 존재하면 수정사항이 생길 시 부모 클래스만 수정해도
        *         전체적으로 적용된다. (유지보수성 증가) / 부모만 변경해도 나머지 자동적으로 변경이 된다.
        * 2. 클래스간의 계층 관계가 형성되며 다형성의 문법적인 토대가 된다. / 형태란 ? 자료형
        *
        * 하지만 상속으로 인한 단점도 존재한다.
        * 1. 부모 클래스의 기능을 추가/변경할 시 자식클래스가 정상적으로 동작하는지에 대한 예측이 힘들다.
        *   상속 구조가 복잡해 질 수록 그 영향에 대한 예측이 힘들며 이러한 단점이 유지보수성 증가한다는 장점과는
        *       반대로 유지보수에 악영향을 미친다.
        * 2. 또한 부모클래스의 변경 또한 쉽지 않다. 자식클래스에서 중요하게 사용하는 기능인 경우
        *   부모클래스를 변경할 시 자식클래스에 모두 영향을 줄 수 있다.
        *   역시 유지보수에 악영향을 미친다.
        * 3. 부모클래스에서는 의미있었던 기능이 자식클래스에서는 무의미할 수 있다.
        *
        * 장점과 단점을 고려했을때,
