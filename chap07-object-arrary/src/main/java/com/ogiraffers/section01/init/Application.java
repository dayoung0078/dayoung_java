...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        * */
        ...
        cars[2] = new Car("페라리",300);
        cars[3] = new Car("페라리",400);
        cars[4] = new Car("페라리",500);

        for (int i = 0; i < cars.length; i++) {  // 5번이라고 하면 안된다. 후에 늘어날 수 있기때문
            cars[i].driveMaxSpeed();
        }

        for (Car car : cars) {  //foreach문
            car = cars[0]; //<-이건 숨겨져있다. 안써도 괜찮음.
            car.driveMaxSpeed();
        }
