...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        * 재사용성을 높일 수 있는 프로그래밍 방식이다. (유지보수성에 안좋은타입)
        * */

        GenericTest<Integer> gt1 = new GenericTest();

        gt1.setValue(10); // 10 입력
        System.out.println(gt1.getValue());
        System.out.println(gt1.getValue() instanceof Integer); // getvalue가 integer랑 같아? (instanceof)
                                                // 상단에 Integer로 지정해줬기 때문에 하단에도 Integer로 써야함


        //제네릭 생성자로 반환,매개변수 타입을 따로 지정하지 않아도 됨~ 꿀인줄 알았는데...
