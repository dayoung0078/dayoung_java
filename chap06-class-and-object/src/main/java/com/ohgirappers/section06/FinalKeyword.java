package com.ohgirappers.section06;
...
public class FinalKeyword {
    ...
    * */
    ...
    // FINAL 상수를 쓸때는 대문자(언더스코어를 사용), 변경 불가, 상수필드에는 초기값을 할당해주지 않으면 에러남 ( 초기값 0)

    private final int NON_STATIC_NUM =10; // 이러면 10이라는 숫자는 죽어도 안변함
    ...
    * */
    ...
    private static final double STATIC_DOUBLE; // 이것만 쓰면 초기값을 정하지않기 때문에 0.0이다
...
//        STATIC_DOUBLE = staticDouble;}
    ...
    * */
    static {
        STATIC_DOUBLE = 0.5; // 근데 여기서 0.5로 지정해줬기때문에 괜찮음
    }

    //클래스의 일반 상수는 생성자를 통하여 초기값을 전달하는것이 가능하다.
    private final String STATIC_STRING;
    //상수 필드는 꼭 초기화를 해야하는 규칙을 갖고있다.
    //그러나 아래와 같이 생성을 하게되면 초기값이 전달이 안되는 것이고
    //결국 상수 필드를 선언한 의미는 없어지게 되는 것이다.

//    public FinalFieldTest() {}


    public FinalKeyword(String staticString) {
